...
router.get('/emails', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load emails', user: req.session });
  }
});
...
router.get('/blog-emails', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load blog emails', user: req.session });
  }
});
...
router.get('/emails/export', (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', `attachment; filename="company-emails-${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csv);
  } catch (error) {
    console.error('Error exporting emails:', error);
    res.status(500).render('error', { error: 'Failed to export emails: ' + error.message, user: req.session });
  }
});
...
router.get('/blog-emails/export', (req, res) => {
  try {
    ...
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="blog-emails-${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csv);
  } catch (error) {
    console.error('Error exporting blog emails:', error);
    res.status(500).render('error', { error: 'Failed to export blog emails: ' + error.message, user: req.session });
  }
});
...
router.post('/emails/new', (req, res) => {
  try {
    const { prospect_id, email, source_page, is_domain_match, is_generic, confidence } = req.body;

    if (!prospect_id || !email) {
      return res.status(400).render('error', { error: 'Prospect and email are required', user: req.session });
    }

    // Check if email already exists
    const existing = db.prepare('SELECT * FROM emails WHERE email = ?').get(email);
    if (existing) {
      return res.status(400).render('error', { error: 'Email already exists', user: req.session });
    }
    ...
    db.prepare('UPDATE prospects SET emails_extracted = 1 WHERE id = ?').run(prospect_id);

    res.redirect(`/prospects/${prospect_id}`);
  } catch (error) {
    console.error('Error creating email:', error);
    res.status(500).render('error', { error: 'Failed to create email: ' + error.message, user: req.session });
  }
});
...
router.get('/emails/:id/edit', (req, res) => {
  try {
    ...
    if (!email) {
      return res.status(404).render('error', { error: 'Email not found', user: req.session });
    }
    ...
    res.status(500).render('error', { error: 'Failed to load email', user: req.session });
  }
});
...
router.get('/emails/:id', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load email details', user: req.session });
  }
});
...
router.post('/emails/:id/update', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to update email: ' + error.message, user: req.session });
  }
});
...
router.post('/emails/:id/delete', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to delete email: ' + error.message, user: req.session });
  }
});
...
router.get('/blog-emails/new', (req, res) => {
  try {
    ...
    const prospect = db.prepare('SELECT * FROM blog_prospects WHERE id = ?').get(blog_prospect_id);
...
router.post('/blog-emails/new', (req, res) => {
  try {
    const { blog_prospect_id, email, source_page, is_domain_match, is_generic, confidence } = req.body;
    ...
    // Check if email already exists
    const existing = db.prepare('SELECT * FROM blog_emails WHERE email = ?').get(email);
    if (existing) {
      return res.status(400).render('error', { error: 'Email already exists', user: req.session });
    }
    ...
    // Mark prospect as having emails
    db.prepare('UPDATE blog_prospects SET emails_extracted = 1 WHERE id = ?').run(blog_prospect_id);

    res.redirect(`/blog-prospects/${blog_prospect_id}`);
  } catch (error) {
    console.error('Error creating blog email:', error);
    res.status(500).render('error', { error: 'Failed to create blog email: ' + error.message, user: req.session });
  }
});
...
router.get('/blog-emails/:id/edit', (req, res) => {
  try {
    ...
    if (!email) {
      return res.status(404).render('error', { error: 'Blog email not found', user: req.session });
    }

    const prospect = db.prepare('SELECT * FROM blog_prospects WHERE id = ?').get(email.blog_prospect_id);
    ...
    res.status(500).render('error', { error: 'Failed to load blog email', user: req.session });
  }
});
...
router.get('/blog-emails/:id', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load blog email details', user: req.session });
  }
});
...
router.post('/blog-emails/:id/update', (req, res) => {
  try {
    ...
    const emailRecord = db.prepare('SELECT blog_prospect_id FROM blog_emails WHERE id = ?').get(id);
    if (emailRecord) {
    ...
    res.status(500).render('error', { error: 'Failed to update blog email: ' + error.message, user: req.session });
  }
});
...
router.post('/blog-emails/:id/delete', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to delete blog email: ' + error.message, user: req.session });
  }
});
