...
router.get('/prospects', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load prospects', user: req.session });
  }
});
...
router.get('/prospects/export', (req, res) => {
  try {
    ...
    const csvRows = prospects.map(p => {
      ...
      const contactPage = (p.contact_pages || '').split(',').map(cp => cp.trim()).filter(cp => cp).shift() || '';
      const contactEmail = (p.all_emails || '').split(',').map(em => em.trim()).filter(em => em).shift() || '';
    ...
    res.setHeader('Content-Disposition', `attachment; filename="prospects-export-${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csv);
  } catch (error) {
    console.error('Error exporting prospects:', error);
    res.status(500).render('error', { error: 'Failed to export prospects: ' + error.message, user: req.session });
  }
});
...
router.post('/prospects/new', (req, res) => {
  try {
    const { domain, company_name, website_url, city, country, campaign_id } = req.body;

    if (!domain) {
      return res.status(400).render('error', { error: 'Domain is required', user: req.session });
    }

    // Check if prospect already exists
    const existing = db.prepare('SELECT * FROM prospects WHERE domain = ?').get(domain);
    if (existing) {
      return res.status(400).render('error', { error: 'Prospect with this domain already exists', user: req.session });
    }
    ...
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?)
    `);
    const result = stmt.run(domain, company_name || null, website_url || null, city || null, country || null);
    ...
    res.status(500).render('error', { error: 'Failed to create prospect: ' + error.message, user: req.session });
  }
});
...
router.get('/prospects/:id/edit', (req, res) => {
  try {
    ...
    if (!prospect) {
      return res.status(404).render('error', { error: 'Prospect not found', user: req.session });
    }
...
router.get('/prospects/:id', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load prospect details', user: req.session });
  }
});
...
router.post('/prospects/:id/update', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to update prospect: ' + error.message, user: req.session });
  }
});
...
router.post('/prospects/:id/delete', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to delete prospect: ' + error.message, user: req.session });
  }
});
...
router.post('/prospects/:id/exclude', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to add to exclusion: ' + error.message, user: req.session });
  }
});
...
router.get('/blog-prospects', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load blog prospects', user: req.session });
  }
});
...
router.get('/blog-prospects/export', (req, res) => {
  try {
    ...
    res.setHeader('Content-Type', 'text/csv');
    res.setHeader('Content-Disposition', `attachment; filename="blog-prospects-export-${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csv);
  } catch (error) {
    console.error('Error exporting blog prospects:', error);
    res.status(500).render('error', { error: 'Failed to export blog prospects: ' + error.message, user: req.session });
  }
});
...
router.post('/blog-prospects/new', (req, res) => {
  try {
    ...
    // Check if prospect already exists
    const existing = db.prepare('SELECT * FROM blog_prospects WHERE domain = ?').get(domain);
    if (existing) {
      return res.status(400).render('error', { error: 'Blog prospect with this domain already exists', user: req.session });
    }
    ...
    res.status(500).render('error', { error: 'Failed to create blog prospect: ' + error.message, user: req.session });
  }
});
...
router.get('/blog-prospects/:id/edit', (req, res) => {
  try {
    const prospect = db.prepare('SELECT * FROM blog_prospects WHERE id = ?').get(req.params.id);
    if (!prospect) {
      return res.status(404).render('error', { error: 'Blog prospect not found', user: req.session });
    }
...
router.get('/blog-prospects/:id', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to load blog prospect details', user: req.session });
  }
});
...
router.post('/blog-prospects/:id/add-to-campaign', (req, res) => {
  try {
    ...
    if (!campaign_id) {
      return res.status(400).render('error', { error: 'Campaign is required', user: req.session });
    }

    // Check if prospect exists
    const prospect = db.prepare('SELECT * FROM blog_prospects WHERE id = ?').get(prospectId);
    if (!prospect) {
      return res.status(404).render('error', { error: 'Blog prospect not found', user: req.session });
    }
    ...
    db.prepare(`
      INSERT INTO blog_leads (brand_id, campaign_id, blog_prospect_id, source_type, source_query)
      VALUES (?, ?, ?, ?, 'manual')
    `).run(campaign.brand_id, campaign_id, prospectId, source_type || 'manual');

    res.redirect(`/blog-prospects/${prospectId}?success=added_to_campaign`);
  } catch (error) {
    console.error('Error adding to campaign:', error);
    res.status(500).render('error', { error: 'Failed to add to campaign: ' + error.message, user: req.session });
  }
});
...
router.post('/blog-prospects/:id/update', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to update blog prospect: ' + error.message, user: req.session });
  }
});
...
router.post('/blog-prospects/:id/delete', (req, res) => {
  try {
    ...
    res.status(500).render('error', { error: 'Failed to delete blog prospect: ' + error.message, user: req.session });
  }
});
